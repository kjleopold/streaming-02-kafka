...
def send_messages(producer: Any, settings: KafkaSettings) -> int:
    """Generate and send enriched messages to the Kafka topic."""
    ...
    try:
        for message in generate_messages(MESSAGE_COUNT):
            # ==========================================
            # ENRICH STREAMING EVENT DATA
            # ==========================================

            # Calculate total revenue for the order
            revenue = float(message["unit_price"]) * int(message["quantity"])
            message["total_revenue"] = round(revenue, 2)

            # Add a simple risk classification
            if revenue >= 100:
                message["sales_tier"] = "PREMIUM"
            elif revenue >= 50:
                message["sales_tier"] = "STANDARD"
            else:
                message["sales_tier"] = "BASIC"
